...
;
function createEventFunc(doclet, funcName, options = { includeComments: true }) {
    const params = [];
    const voidTypeNode = ts.createKeywordTypeNode(ts.SyntaxKind.VoidKeyword);
    const optionalParams = options.optionalParams;
    const [eventName] = type_resolve_helpers_1.resolveOptionalFromName(doclet);
    let questionToken = optionalParams === 'all' ? ts.createToken(ts.SyntaxKind.QuestionToken) : undefined;
    params.push(ts.createParameter(undefined, undefined, undefined, 'event', questionToken, type_resolve_helpers_1.resolveType({ names: [`'${eventName}'`] }), undefined));
    questionToken = optionalParams === 'all' || optionalParams === 'last' ? ts.createToken(ts.SyntaxKind.QuestionToken) : undefined;
    params.push(ts.createParameter(undefined, undefined, undefined, 'callback', questionToken, ts.createFunctionTypeNode(undefined, type_resolve_helpers_1.createFunctionParams(doclet), voidTypeNode), undefined));
    const methodDeclaration = ts.createMethod(undefined, [], undefined, funcName, undefined, [], params, voidTypeNode, undefined);
